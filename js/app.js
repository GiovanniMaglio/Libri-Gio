...
(function () {
  ...
  var COVER_CLASSES = ["forest", "oxblood", "navy", "ochre", "walnut", "plum"];
  ...
  function coverClassFor(genere) {
    var key = (genere || "senza genere").trim().toLowerCase();
    return COVER_CLASSES[hashString(key) % COVER_CLASSES.length];
  }
  ...
  function renderGrid(books) {
    ...
    els.grid.innerHTML = books.map(function (b) {
      var cls = coverClassFor(b.genere);
      var label = escapeHtml(b.title) + (b.author ? ", " + escapeHtml(b.author) : "");
      return '<button type="button" class="cover ' + cls + '" ' +
