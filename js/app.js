/* La mia libreria — app.js
   ...
   gestisce ricerca/filtro e la pagina di dettaglio via hash routing (#/libro/slug).

   Schema di un libro in data/books.json — solo "titolo" è obbligatorio:
   { "titolo": "...", "autore": "...", "voto": 4.5, "genere": "...",
     "recensione": "...", "data": "2026-08-09" }
   Non serve un "id": viene generato da titolo/autore automaticamente. */
...
(function () {
  ...
  function buildGenreColors(genres) {
    ...
    genres.forEach(function (g, i) {
      ...
      map[g] = {
        c: "hsl(" + hue + " 30% 28%)",   // sfondo copertina: caldo, profondo, mai acceso
        t: "hsl(" + hue + " 42% 90%)"    // tinta chiara per il tag genere nel dettaglio
      };
    });
    return map;
  }
  ...
  // I libri aggiunti/letti più di recente in cima. "data" è una stringa
  // AAAA-MM-GG (o vuota): a parità di data si mantiene l'ordine del file.
  function sortByRecent(books) {
    return books.slice().sort(function (a, b) {
      var da = new Date(a.data || 0).getTime();
      var db = new Date(b.data || 0).getTime();
      return db - da;
    });
  }

  // Genera un identificativo leggibile dal titolo (usato nell'URL della
  // pagina di dettaglio), evitando doppioni se due libri hanno lo stesso
  // titolo esatto.
  function assignSlugs(books) {
    var used = {};
    books.forEach(function (b) {
      var base = String(b.titolo || "libro")
        .toLowerCase()
  ...
  function renderGrid(books) {
    ...
    els.grid.innerHTML = books.map(function (b) {
      var label = escapeHtml(b.titolo) + (b.autore ? ", " + escapeHtml(b.autore) : "");
      return '<button type="button" class="cover" style="' + genreStyleAttr(b.genere) + '" ' +
        'data-id="' + escapeHtml(b._id) + '" aria-label="Apri la recensione di ' + label + '">' +
        '<span class="cover-genre">' + escapeHtml(b.genere || "Senza genere") + '</span>' +
        '<span class="cover-title">' + escapeHtml(b.titolo) + '</span>' +
        '<span class="cover-author">' + escapeHtml(b.autore || "") + '</span>' +
        '<span class="cover-rating">' + ratingDotsHtml(b.voto) + '</span>' +
        '</button>';
    }).join("");
  }
  ...
  function applyFilters() {
    ...
    state.filtered = state.books.filter(function (b) {
      ...
      if (!q) return true;
      var haystack = [b.titolo, b.autore, b.recensione].filter(Boolean).join(" ").toLowerCase();
  ...
  function findBook(id) {
    var idStr = String(id);
    return state.books.filter(function (b) { return b._id === idStr; })[0];
  }
  ...
  function openDetail(book) {
    ...
    els.detailContent.innerHTML =
      ...
      '<h2 class="detail-title" id="detail-title">' + escapeHtml(book.titolo) + '</h2>' +
      '<p class="detail-author">' + escapeHtml(book.autore || "Autore sconosciuto") + '</p>' +
      '<div class="detail-rating">' +
        '<span class="dots">' + ratingDotsHtml(book.voto) + '</span>' +
        '<span class="rating-number">' + (book.voto != null ? Number(book.voto).toFixed(1) + " / 5" : "Nessun voto") + '</span>' +
      '</div>' +
      (book.recensione ? '<p class="detail-review">' + escapeHtml(book.recensione) + '</p>' : '') +
      '<p class="detail-meta">Aggiunto il ' + escapeHtml(formatDate(book.data) || "data sconosciuta") + '</p>';
